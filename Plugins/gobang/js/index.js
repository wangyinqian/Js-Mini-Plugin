{
    class Gobang {
        ...
        updated(){
            if(this.state)
            { 
                this.rule(this.col,this.$vue.rows[this.col.rowIndex],this.$vue.rows);
    
                this.current = this.current == "black" ? "white" : "black";
                
                this.col = null,this.state = false; 
            }
        }
        initData(){
        ...
        created(event){
           ...
           for(let i = 0,len = this.$vue.rows.length;i<len;i++)
           {
               ...
               for(let j = 0,count = _row.length;j < count;j++)
               {
                    ...
                    {
                        return this.col = Object.assign(_col,{type:this.current,index:j,rowIndex:i}),this.state = true; 
                    }
               }
           }  
        }
